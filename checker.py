    ...
    for form in json_files:
        ...
        with open(form, 'r') as f:
            ...
            for concept_key, uuid in concept_uuids:
                ...
                response = requests.get(f'http://msf-ocg-openmrs3-dev.westeurope.cloudapp.azure.com/openmrs/ws/rest/v1/concept/{uuid}', auth=('admin', 'Admin123'))
                if response.status_code == 200:
                    # Get value of key 'display' from response
                    name = response.json().get('display')
                    concept_status.append({'form': form, 'uuid': uuid, 'status': 'Exists', 'name': name})
                    print(f'Concept {name} with UUID {uuid} exists in the OpenMRS database')
                else:
                    name = 'not found'
                    concept_status.append({'form': form, 'uuid': uuid, 'status': 'Not found'})
