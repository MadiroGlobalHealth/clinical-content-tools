...
def check_and_add_reference(concept_details):
    ...
    if response.status_code == 200:
        print(
            f"Concept '{concept['name']}' already exists in OCL collection - Skipping"
        )
    else:
        # Add the newly created concept to the OCL collection as a reference
        reference_query = {
            "data": {"expressions": [concept_details["url"]]},
            "cascade": {
        ...
        response = requests.put(
            collection_api_url, json=reference_query, headers=HEADERS
        )
        if response.status_code == 200:
            print(f"Concept '{concept['name']}' added to OCL collection as a reference")
        else:
            print(
                f"Failed to add concept '{concept['name']}' to OCL collection - Skipping reference"
            )
...
def create_concepts(csv_file):
    ...
    for _, row in df.iterrows():
        concept = {
            "name": row["Name"],
            "external_id": (
                row["External ID"] if not pd.isnull(row["External ID"]) else None
            ),
        }
...
for concept in concepts:
    ...
    if concept["external_id"] is None:
        ...
        if response.status_code == 201:
            ...
            df.loc[df["Name"] == concept["name"], "External ID"] = concept[
                "external_id"
            ]
            # Update the CSV file with external IDs if concept creation is successful
            df.to_csv(csv_file, index=False)
            print(
                f"Concept '{concept['name']}' created successfully with External ID: {concept['external_id']}"
            )
        ...
        print(
            f"Concept {concept['name']} already exists with external ID: {concept['external_id']}"
        )
